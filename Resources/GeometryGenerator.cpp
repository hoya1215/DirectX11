...
MeshData GeometryGenerator::MakeCircle(const float scale,
    const Vector2 texScale)
{
    vector<Vector3> positions;
    vector<Vector3> colors;
    ...
    Vector3 startPoint = Vector3(0.0f, -scale, 0.0f);

    Vertex v1;
    v1.position = center;
    v1.texcoord = Vector2(0.5f, 0.5f);

    float angle = XM_2PI / 30.0f;
    ...
    for (int i = 0; i < 30; i++)
    {
        Vertex v;
        v.position =
            Vector3::Transform(startPoint, Matrix::CreateRotationZ(angle * i));
        v.texcoord.x =  0.5f * (v.position.x / scale + 1.0f );
        v.texcoord.y = 1- 0.5f * (v.position.y / scale  + 1.0f);
...
MeshData GeometryGenerator::MakeWireBBox(const Vector3 center,
    ...
    for (size_t i = 0; i < v.size(); i++)
    {
        Vertex p;
        p.position = v[i];
        p.normalModel = v[i] - center;
        p.normalModel.Normalize();
        p.texcoord = Vector2(0.0f, 0.0f); // 미사용
        meshdata.vertices.push_back(p);
...
auto GeometryGenerator::ReadAnimationFromFile(string basePath, string filename,
    ...
    return {modelLoader.meshes, modelLoader.m_aniData};
}