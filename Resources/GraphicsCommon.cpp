...
ComPtr<ID3D11PixelShader> basicDeferredPS;
...
GraphicsPSO defaultDeferredPSO;
...
ComPtr<ID3D11ComputeShader> m_ssrCS;
...
void Graphics::InitShaders(ComPtr<ID3D11Device> &device) {
    ...
    D3D11Utils::CreatePixelShader(
     ...
     D3D11Utils::CreatePixelShader(
         device, L"BasicPS.hlsl", basicCharacterPS,   
         vector<D3D_SHADER_MACRO>{{"SKINNED", "1"},{"DEFERRED", "1"},{NULL, NULL}});
    D3D11Utils::CreatePixelShader(
        device, L"BasicPS.hlsl", basicDeferredPS,
         vector<D3D_SHADER_MACRO>{{"DEFERRED", "1"},
                                  {"OBJECT", "1"}, {NULL, NULL}});
    ...
    D3D11Utils::CreateComputeShader(device, L"SSRCS.hlsl", m_ssrCS);
}
...
void Graphics::InitPipelineStates(ComPtr<ID3D11Device> &device) {
    ...
    // Deferred
    defaultDeferredPSO = defaultSolidPSO;
    defaultDeferredPSO.m_pixelShader = basicDeferredPS;
    ...
    postEffectsPSO.m_blendState = alphaBS;
