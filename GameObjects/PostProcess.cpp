...
void PostProcess::Initialize(
    ...
    m_combineFilter.m_constData.dof = false; // dof 
    m_combineFilter.m_constData.option1 = 1.0f;  // Exposure로 사용
    m_combineFilter.m_constData.option2 = 2.2f;  // Gamma로 사용
    m_combineFilter.m_constData.dofLevel = 0.0f;
    m_combineFilter.UpdateConstantBuffers(device, context);  
} 
  
void PostProcess::Render(ComPtr<ID3D11DeviceContext> &context, ComPtr<ID3D11ShaderResourceView> depthSRV) {

    context->PSSetSamplers(10, 1, Graphics::linearClampSS.GetAddressOf());
    context->PSSetShaderResources(2, 1, depthSRV.GetAddressOf());
    ...
    // 블룸이 필요한 경우에만 계산
    if (m_combineFilter.m_constData.dof) {
