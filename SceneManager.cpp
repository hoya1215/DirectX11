...
#pragma endregion
    ...
    {
        g_buffer[i] =
            make_shared<UI>(m_device, m_context,
                            Vector3(300.0f - 200.0f * i, 300.0f, 100.0f), 50.0f);
        g_buffer[i]->m_constantData.index = 3;
        g_buffer[i]->Update(m_device, m_context);
    }

    // Screen Space
    m_ssr.Initialize(m_device, m_context);
...
void SceneManager::UpdateGUI() {
        ...
        if (ImGui::TreeNode("Function")) {
            ...
            ImGui::Checkbox("G_Buffer", &b_gbuffer);

            int flag = 0;
            flag += ImGui::Checkbox(
                "DOF", &m_postProcess.m_combineFilter.m_constData.dof);
            if (m_postProcess.m_combineFilter.m_constData.dof)
            {
            flag += ImGui::SliderFloat(
                "DOF Level",
                &m_postProcess.m_combineFilter.m_constData.dofLevel, 0.0f,
                16.0f);
            }
            if (flag)
            ...
            int flag2 = 0;
            flag2 += ImGui::Checkbox("SSR", &m_postEffectsConstsCPU.ssr);
            if (m_postEffectsConstsCPU.ssr)
            { 
                

                flag2 += ImGui::SliderFloat("Count", &m_ssr.m_constantData.MaxCount, 0.0f,
                               60.0f);
            flag2 += ImGui::SliderFloat(
                "Ray Dist", &m_ssr.m_constantData.RayDist, 0.0f,
                               1.0f);
                flag2 += ImGui::SliderFloat(
                    "Z Dist", &m_ssr.m_constantData.ZDist, 0.01f,
                                   0.1f);
                
                
            }
            if (flag2)
                m_ssr.UpdateBuffer(m_device, m_context);
...
void SceneManager::RenderBasicScene() {
    ...
    m_context->OMSetRenderTargets(UINT(rtvs2.size()), rtvs2.data(), 
        ...
        AppBase::SetPipelineState(Graphics::defaultDeferredPSO);
...
void SceneManager::TerrainMapPass()
{
        ...
        AppBase::SetPipelineState(Graphics::terrainSolidPSO);
        m_context->PSSetShader(Graphics::basicDeferredPS.Get(), 0, 0);
        m_terrain.Render(m_context);
...
{   
        ...
        vector<ID3D11RenderTargetView *> rtvs = {
            m_EditRTV.Get(), m_positionRTV.Get(), m_normalRTV.Get(), m_depthRTV.Get()};
...
#pragma region 실루엣
        ...
        AppBase::SetPipelineState(m_drawAsWire ? Graphics::terrainWirePSO
                                               : Graphics::terrainSolidPSO); 
        //m_context->PSSetShader(Graphics::basicForwardPS.Get(), 0, 0);
        AppBase::SetGlobalConsts(m_globalConstsGPU);
...
#pragma endregion 
       ...
       // 투명도
        ...
        if (m_postEffectsConstsCPU.ssr)
        {
            vector<ID3D11ShaderResourceView *> ssSRVs = {
                m_resolvedSRV[0].Get(), m_resolvedSRV[1].Get(),
                m_resolvedSRV[2].Get(), m_resolvedSRV[3].Get()};
            m_context->CSSetShaderResources(0, UINT(ssSRVs.size()),
                                            ssSRVs.data());
            m_ssr.Render(m_context, Graphics::m_ssrCS); 
        }
...
#pragma region UI 
        ...
        m_map->Render(m_context, m_terrainMapSRV);
            ...
            m_context->ResolveSubresource(m_resolvedBuffer[1].Get(), 0,
                                      m_positionBuffer.Get(), 0,
                                      DXGI_FORMAT_R16G16B16A16_FLOAT);
        m_context->ResolveSubresource(m_resolvedBuffer[2].Get(), 0,
                                      m_normalBuffer.Get(), 0,
                                      DXGI_FORMAT_R16G16B16A16_FLOAT); 

        if (b_gbuffer) {
            g_buffer[0]->Render(m_context, m_resolvedSRV[3]); 
            g_buffer[1]->Render(m_context, m_resolvedSRV[2]);
            g_buffer[2]->Render(m_context, m_resolvedSRV[1]);
           // g_buffer[3]->Render(m_context, m_ssr.m_ssSRV);
        }
...
void SceneManager::RenderPostEffect() {
        if (EditScene) {
            ...
            //m_floatRTV = m_positionRTV;
            //m_floatBuffer = m_positionBuffer;
        m_context->ResolveSubresource(m_resolvedBuffer[0].Get(), 0,
                                          m_EditBuffer.Get(), 0,
                                          DXGI_FORMAT_R16G16B16A16_FLOAT);
            m_context->ResolveSubresource(m_resolvedBuffer[1].Get(), 0, 
                                          m_positionBuffer.Get(), 0,
        ...
        m_context->ResolveSubresource(m_resolvedBuffer[2].Get(), 0,
                                      m_normalBuffer.Get(), 0,
                                      DXGI_FORMAT_R16G16B16A16_FLOAT);
            m_context->ResolveSubresource(m_resolvedBuffer[3].Get(), 0,
                                          m_depthBuffer.Get(), 0,
                                          DXGI_FORMAT_R16G16B16A16_FLOAT);
        } else {
        ...
        vector<ID3D11ShaderResourceView *> postEffectsSRVs = {
            m_resolvedSRV[0].Get(), m_resolvedSRV[1].Get(),
            m_resolvedSRV[2].Get(),
            m_resolvedSRV[3].Get(), m_ssr.m_ssSRV.Get()};
        ...
        m_postProcess.Render(m_context, m_resolvedSRV[3]); 
}
