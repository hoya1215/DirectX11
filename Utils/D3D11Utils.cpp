    ...
    const vector<D3D_SHADER_MACRO> shaderMacros) 
{
    ...
    // 셰이더의 시작점의 이름이 "main"인 함수로 지정
    // D3D_COMPILE_STANDARD_FILE_INCLUDE 추가: 셰이더에서 include 사용
...
void D3D11Utils::CreatePixelShader(ComPtr<ID3D11Device> &device,
    ...
    // D3D_COMPILE_STANDARD_FILE_INCLUDE 추가: 셰이더에서 include 사용
    HRESULT hr = D3DCompileFromFile(
        filename.c_str(), shaderMacros.empty() ? NULL : shaderMacros.data(), D3D_COMPILE_STANDARD_FILE_INCLUDE, "main",
...
void D3D11Utils::CreateTextureUAV(ComPtr<ID3D11Device> &device, 
                           ComPtr<ID3D11Texture2D> &texture,
    ...
    device->CreateUnorderedAccessView(texture.Get(), &uavDesc,
                                      uav.GetAddressOf());
}
...
void D3D11Utils::CreateTexture2D(ComPtr<ID3D11Device>& device, ComPtr<ID3D11DeviceContext> &context,
    ...
    ComPtr<ID3D11UnorderedAccessView>& uav)
{
    ...
    device->CreateTexture2D(&desc, NULL, buffer.GetAddressOf());
...
void D3D11Utils::CreateStagingTexture2(ComPtr<ID3D11Device>& device,
    ComPtr<ID3D11DeviceContext>& context,
    ComPtr<ID3D11Texture2D>& buffer)
{
    D3D11_TEXTURE2D_DESC desc;
    ...
    desc.Usage = D3D11_USAGE_STAGING; // 스테이징 텍스춰로부터 복사 가능
    desc.MiscFlags = 0;
    desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE | D3D11_CPU_ACCESS_READ;
    desc.SampleDesc.Count = 1;
    desc.SampleDesc.Quality = 0;
...
void D3D11Utils::CreateResolveTexture2(ComPtr<ID3D11Device> &device,
                                        ComPtr<ID3D11DeviceContext> &context,
                                        ComPtr<ID3D11Texture2D> &buffer) {
        D3D11_TEXTURE2D_DESC desc;
        desc.MipLevels = 1;
