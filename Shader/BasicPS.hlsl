...
PixelShaderOutput main(PixelShaderInput input)
{
    ...
    color = float4(albedo, 1.0) * ao;


    // 햇빛
    float3 sunRadiance =
        BasicLightRadiance(Sun, input.normalWorld, input.posWorld,
                           ambientC, diffuseC, specularC, SunShadowMap);

    output.pixelColor = float4(color * sunRadiance, 1.0);
